# -*- coding: utf-8 -*-
"""
Created on Tue Mar 15 19:21:43 2022
...
transitions_df = pd.read_csv(r'C:\Users\44743\Documents\Imperial Year 4\MSci Project\Catalogues\hydrogencolumns.csv', skiprows=1, names=column_names)
...
#plt.plot(masked_Ha_reg, norm_spectra*300, label = f"{filename}", color = 'darkblue') #HF # plot the normalised spectrum

for i in range(len(alpha_array)):
    plt.plot(wavelength_alpha_list[i],b_alpha_list[i]/1e6, '--', label=labels_alpha[i])
for i in range(len(alpha_array)):
    plt.plot(wavelength_alpha_list[i],b_alpha_list[i]/1e6,'x')
#for i in range(len(beta_array)):
#    plt.plot(wavelength_beta_list[i],b_beta_list[i]/1e6, label=labels_beta[i])
#for i in range(len(beta_array)):
#    plt.plot(wavelength_beta_list[i],b_beta_list[i]/1e6,'x')

#plt.plot(masked_Gauss_reg, norm_Gauss_spectra*33.4, label = f"{filename}", color = 'darkblue') # Quad #plot the normalised spectrum
#plt.plot(masked_Ha_reg, norm_spectra*300, label = f"{filename}", color = 'darkblue') #HF # plot the normalised spectrum

#plt.plot(masked_Ha_reg, norm_spectra*33.4, label = f"{filename}", color = 'darkblue') #HF # plot the normalised spectrum

#lambda_values = [6155, 6624, 6785, 7204, 6624, 6785, 7204, 8551]
#for i in range(len(lambda_values)):
#    plt.plot(np.array([lambda_values[i]]*100), np.linspace(0, 960, 100))

plt.xticks(fontsize=11)
plt.yticks(fontsize=11)
plt.xlabel(r'Wavelength $(\mathrm{\AA})$', fontsize = "13", labelpad = 9)
plt.ylabel('Magnetic Field Strength (MG)', fontsize = "13", labelpad = 10)
...
# HF regime trans plot
#plt.xlim(2730, 9850)
#plt.ylim(-20, 960)

# HF regime trans overplot
plt.xlim(4015, 9450)
plt.ylim(-20, 667)

# HF transitions report plot (method)
#plt.xlim(4760 ,8670)
#plt.ylim(-8, 360)

#plt.savefig(f'HF transitions overplot.png', dpi = 1000, bbox_inches = 'tight')
#plt.savefig(f'HF overplot report.pdf', bbox_inches = 'tight')
