...
input_table = pd.read_csv('chisq left pi input table 2.csv')
filename = "DESI_WDJ002413.22+182405.17_bin0p2.dat"
...
print(df_system)
p_sigma_left = [round( df_system.iloc[0][2] , -1) , round( df_system.iloc[0][3], -1), 3]
p_right_pi = [round( df_system.iloc[0][4] , -1) , round( df_system.iloc[0][5], -1), 3]
p_sigma_right = [round( df_system.iloc[0][6] , -1) , round( df_system.iloc[0][7], -1), 3]
#%%
...
begin = df_system.iloc[0][8]
finish = df_system.iloc[0][9]
start_Gauss_1 = df_system.iloc[0][10]
end_Gauss_1 = df_system.iloc[0][11]
start_Gauss_2 = 6510
end_Gauss_2 = 6562
start_Gauss_3 = df_system.iloc[0][14]
end_Gauss_3 = df_system.iloc[0][15]
start_Gauss_4 = 6363
end_Gauss_4 = 6437
...
dip_start4 = int(np.where(masked_Gauss_reg == min(masked_Gauss_reg, key=lambda x:abs(x-start_Gauss_4)))[0])
dip_end4 = int(np.where(masked_Gauss_reg == min(masked_Gauss_reg, key=lambda x:abs(x-end_Gauss_4)))[0])
...
flxframe.loc[dip_start4:dip_end4] = np.nan
...
wavframe.loc[dip_start4:dip_end4] = np.nan
...
for a in range(len(masked_Gauss_reg)):
    ...
    if masked_Gauss_reg[a] > (start_Gauss_4) and masked_Gauss_reg[a] < (end_Gauss_4):
        x4_feature.append(masked_Gauss_reg[a])
        y4_feature.append(norm_Gauss_spectra[a])
        err4_feature.append(masked_err[a])
...
x4_feature = np.array(x4_feature)
y4_feature = np.array(y4_feature)
err4_feature = np.array(err4_feature)
...
popt_Gauss_4, cov_Gauss_4 = opt.curve_fit(Gaussian, x4_feature, y4_feature, p4, sigma = err4_feature)
...
for c in zip(popt_Gauss_4, np.sqrt(np.diag(cov_Gauss_4))):
    print("%.8f pm %.3g" % (c[0], c[1])) 
...
plt.savefig('Intermediate Field Gaussian Fit Plots with Left Pi/'+f'{filename}'+'_sigma_minus.png')

plt.figure("Second Gaussian")
plt.grid()
plt.plot(x2_feature,y2_feature)
plt.plot(x2_feature,Gaussian(x2_feature,*popt_Gauss_2))
plt.savefig('Intermediate Field Gaussian Fit Plots with Left Pi/'+f'{filename}'+'_pi.png')

plt.figure("Third Gaussian")
plt.grid()
plt.plot(x3_feature,y3_feature)
plt.plot(x3_feature,Gaussian(x3_feature,*popt_Gauss_3))
plt.savefig('Intermediate Field Gaussian Fit Plots with Left Pi/'+f'{filename}'+'_sigma_plus.png')

plt.figure("Fourth Gaussian")
plt.grid()
plt.plot(x4_feature,y4_feature)
plt.plot(x4_feature,Gaussian(x4_feature,*popt_Gauss_4))
plt.savefig('Intermediate Field Gaussian Fit Plots with Left Pi/'+f'{filename}'+'_pi_left.png')
#%%
...
left_pi_sq = np.square(wavelength_output[5]-popt_Gauss_4[0])
...
left_pi_chi = (1/(popt_Gauss_4[1]))*left_pi_sq
...
left_pi_chi_2 = (1/(popt_Gauss_4[0]))*left_pi_sq

chi_tot = left_sigma_chi+pi_chi+right_sigma_chi
chi_tot_2 = left_sigma_chi_2+pi_chi_2+right_sigma_chi_2
chi_tot_3 = left_sigma_chi+pi_chi+right_sigma_chi+left_pi_chi
chi_tot_4 = left_sigma_chi_2+pi_chi_2+right_sigma_chi_2+left_pi_chi_2
...
chi_B_guess_4 = B_range[chi_min_idx_4]
