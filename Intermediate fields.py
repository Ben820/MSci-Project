...
import pandas as pd 
import re
""" NEED TO RUN hydrogen_transitions_v3.py BEFORE RUNNING ANY CODE IN THIS FILE"""
...
filename = "DESI_WDJ013224.10+013558.03_bin0p2.dat"
filename = "DESI_WDJ210911.96+040435.94_bin0p2.dat"
...
for i in range(len(b_alpha_list)):
    cs_list.append( spi.CubicSpline( b_alpha_list[i][start_list[i]-3:end_list[i]+3], \
                                    wavelength_alpha_list[i][start_list[i]-3:end_list[i]+3] ) )
...
## r"$\langle B \rangle = $"
#print(' Average B =', np.mean([left_B_guess, pi_B_guess, right_B_guess])/1e6)
##%%
# =============================================================================
"""
Chi-squared fitting
...
# 
chi_B_guess = B_range[chi_min_idx]/1e6
chi_B_guess_2 = B_range[chi_min_idx_2]/1e6
# 
print(chi_B_guess)
print(chi_B_guess_2)
print(chi_min)
print(chi_min_2)
##%%
dataIFlist = [popt_Gauss_1[0], popt_Gauss_1[1], popt_Gauss_2[0], popt_Gauss_2[1], popt_Gauss_3[0],\
              popt_Gauss_3[1], chi_B_guess, chi_B_guess_2, chi_min, chi_min_2, begin, finish, start_Gauss_1, \
              end_Gauss_1, start_Gauss_2, end_Gauss_2, start_Gauss_3, end_Gauss_3]
dataIFlist2 = [chi_B_guess_2, 0.05*chi_B_guess_2]
##%%
cexceldata = np.zeros((1,18)) # 1,24

for i in range(0, len(dataIFlist)):
    cexceldata[:,i] = dataIFlist[i]
    
bfexceldata = np.zeros((1,2))

for i in range(0, 2):
    bfexceldata[:,i] = dataIFlist2[i]
#%%
# """
